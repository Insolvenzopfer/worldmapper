...
async function init() {
  ...
  try {
    ...
    if (ds.region_label_width) {
      ...
      console.log("Region Label Breite gesetzt auf:", _REG_LABEL_W); // Zur Kontrolle
    }
  ...
  await loadPoiIcons();
  try {
    _customIcons = await API.get("/api/icons");
  } catch {}
  initFogCanvas();
  renderSidebar();
  renderAllFeatures();
  ...
  if (isAdmin && fogCanvas && _fogDensity !== 1.0)
...
function initLeaflet(map) {
  const W = map.image_width || 2000,
    H = map.image_height || 2000;
...
function bucketsFor(item) {
  if (item.visibility === "public")
    return { bucket: PUB, layers: globalLayers };
  if (item.visibility === "hidden")
    return { bucket: HID, layers: hiddenLayers };
...
function initFogCanvas() {
  ...
  (mapData.fog_areas || []).forEach((fa) => {
    const coords = Array.isArray(fa.coordinates)
      ? fa.coordinates
      : typeof fa.coordinates === "string"
        ? JSON.parse(fa.coordinates)
        : [];
...
function toggleLayer(groupId, type, btn) {
  ...
  if (type === "visible") {
    ...
    if (newVal && !ls.visible) {
      ...
      const vBtn = document.querySelector(
        `[data-gid="${groupId}"][data-type="visible"]`,
      );
  ...
  if (_showPoiLabels && (type === "visible" || type === "poi"))
    _renderPoiLabels();
  if (_showRegLabels && (type === "visible" || type === "region"))
    _renderRegionLabels();
}
...
function _setGroupSubs(groupId, on, savedSubs = null) {
  ...
  ["poi", "route", "region"].forEach((t) => {
    ...
    if (lay)
      val && ls.visible
        ? leafletMap.addLayer(lay)
        : leafletMap.removeLayer(lay);
    const btn = document.querySelector(
      `[data-gid="${groupId}"][data-type="${t}"]`,
    );
  ...
  const fogBtn = document.querySelector(
    `[data-gid="${groupId}"][data-type="fog"]`,
  );
  ...
  if (type === "visible") {
    if (!newVal) {
      ...
      ["poi", "route", "region"].forEach((t) => {
        ...
        st[t] = val;
        val
          ? leafletMap.addLayer(layers[t + "Layer"])
          : leafletMap.removeLayer(layers[t + "Layer"]);
      });
    }
...
function makePoiIcon(color, iconKey, transparent) {
  ...
  const customIcon =
    !isNaN(custId) && _customIcons.find((ic) => ic.id === custId);
  let innerHtml;
  if (customIcon) {
    const _isUrl =
      customIcon.image_url.startsWith("http") ||
      customIcon.image_url.startsWith("/");
    innerHtml = _isUrl
    ...
    ${
      links.length
        ? `<div class="popup-links">${links
            .map(
              (l) =>
                `<a href="${l.url}" target="_blank">${escHtml(l.label || l.url)}</a>`,
            )
            .join("")}</div>`
        : ""
    }
...
function removePoi(id) {
  ...
  const lay =
    e.bucket === PUB
      ? globalLayers.poiLayer
      : e.bucket === HID
        ? hiddenLayers.poiLayer
...
async function deletePoi(id) {
  ...
  try {
    await API.delete(`/api/maps/${mapData.map.id}/pois/${id}`);
  } catch (e) {
...
function addRoute(route, waypoints) {
  ...
  const b = bucketsFor({
    ...
    visibility: route.visibility || "group",
  });
  if (!b || !waypoints.length) return;

  const coords = waypoints
    .map((w) => [parseFloat(w.lat), parseFloat(w.lng)])
    .filter((c) => isFinite(c[0]) && isFinite(c[1]));
  if (!coords.length) return;

  const display =
    route.smooth !== false && coords.length >= 3 ? catmullRom(coords) : coords;
  ...
  waypoints.forEach((wp, idx) => {
    ...
    const m = L.marker([lat, lng], {
      icon: makeWpIcon(route.color, isLast),
...
function removeRoute(id) {
  ...
  const lay =
    e.bucket === PUB
      ? globalLayers.routeLayer
      : e.bucket === HID
        ? hiddenLayers.routeLayer
...
async function deleteRoute(id) {
  ...
  leafletMap.closePopup();
  try {
    await API.delete(`/api/maps/${mapData.map.id}/routes/${id}`);
  } catch (e) {
    showToast(e.message, "error");
  }
}
...
function rePickWaypointPos(routeId, wpId) {
  ...
  enablePickMode();
  showModeIndicator(
    "📍 Klick auf Karte für neue Wegpunkt-Position · ESC abbrechen",
  );
  leafletMap.once("click", async (ev) => {
    disablePickMode();
    hideModeIndicator();
    try {
      await API.put(
        `/api/maps/${mapData.map.id}/routes/${routeId}/waypoints/${wpId}`,
        {
          lat: ev.latlng.lat,
          lng: ev.latlng.lng,
          title: wp.title || "",
...
function addRegion(region) {
  ...
  const poly = L.polygon(coords, {
    ...
    fillOpacity: parseFloat(region.fill_opacity) || 0.2,
    opacity: parseFloat(region.stroke_opacity) || 0.8,
    weight: 2,
  });
  const adminBtns = isAdmin
    ? `
  ...
  const baseFill = parseFloat(region.fill_opacity) || 0.2;
  poly.on("mouseover", () =>
    poly.setStyle({ fillOpacity: Math.min(baseFill + 0.1, 0.9) }),
  );
  poly.on("mouseout", () => poly.setStyle({ fillOpacity: baseFill }));

  b.layers.regionLayer.addLayer(poly);
  regionLayers[region.id] = {
    poly,
    bucket: b.bucket,
    groupId: region.group_id,
  };
}
...
function removeRegion(id) {
  ...
  const lay =
    e.bucket === PUB
      ? globalLayers.regionLayer
      : e.bucket === HID
        ? hiddenLayers.regionLayer
...
async function deleteRegion(id) {
  ...
  leafletMap.closePopup();
  try {
    await API.delete(`/api/maps/${mapData.map.id}/regions/${id}`);
  } catch (e) {
    showToast(e.message, "error");
  }
}
...
function joinSocket(mapId) {
  ...
  socket.on("poi:created", (p) => {
    if (!mapData.pois.find((x) => x.id === p.id)) {
      mapData.pois.push(p);
      addPoi(p);
      if (_showPoiLabels) _renderPoiLabels();
      refreshEditorContent();
  ...
  socket.on("poi:updated", (p) => {
    mapData.pois = mapData.pois.map((x) => (x.id === p.id ? p : x));
    removePoi(p.id);
    addPoi(p);
    if (_showPoiLabels) _renderPoiLabels();
    refreshEditorContent();
  });
  socket.on("poi:deleted", ({ id }) => {
    mapData.pois = mapData.pois.filter((x) => x.id !== id);
    removePoi(id);
    if (_showPoiLabels) _renderPoiLabels();
    refreshEditorContent();
  });
  ...
  socket.on("route:created", (r) => {
    if (!mapData.routes.find((x) => x.id === r.id)) {
      mapData.routes.push(r);
      addRoute(r, r.waypoints || []);
      refreshEditorContent();
    }
  });
  socket.on("route:updated", (r) => {
    ...
    const wps =
      r.waypoints || mapData.waypoints.filter((w) => w.route_id === r.id);
    removeRoute(r.id);
    addRoute(r, wps);
    refreshEditorContent();
  });
  socket.on("route:deleted", ({ id }) => {
    mapData.routes = mapData.routes.filter((x) => x.id !== id);
    removeRoute(id);
    refreshEditorContent();
  });
  socket.on("waypoints:updated", ({ route_id, waypoints }) => {
    mapData.waypoints = mapData.waypoints
      .filter((w) => w.route_id !== route_id)
      .concat(waypoints);
    const route = mapData.routes.find((r) => r.id === route_id);
    if (route) {
      removeRoute(route_id);
      addRoute(route, waypoints);
    }
    refreshEditorContent();
  });

  socket.on("region:created", (r) => {
    if (!mapData.regions.find((x) => x.id === r.id)) {
  ...
  socket.on("region:updated", (r) => {
    mapData.regions = mapData.regions.map((x) => (x.id === r.id ? r : x));
    removeRegion(r.id);
    addRegion(r);
    if (_showRegLabels) _renderRegionLabels();
    refreshEditorContent();
  });
  socket.on("region:deleted", ({ id }) => {
  ...
  socket.on("fog:group:cleared", ({ group_id }) => {
    mapData.fog_areas = (mapData.fog_areas || []).filter(
      (a) => a.group_id !== group_id,
    );
    fogCanvas.clearGroup(group_id);
    refreshEditorContent();
  });
  socket.on("map:updated", (m) => {
    mapData.map = m;
  ...
  socket.on("map:locked", (data) => {
    ...
    if (me && me.is_superadmin) {
      showToast(
        `🔒 Karte gesperrt von ${data.locked_by_name} – du siehst sie als Betrachter`,
        "info",
      );
    }
  });
  ...
  socket.on("feature:ping", (data) => {
    ...
    if (data.type === "cursor") {
      ...
      if (data.moveView && !isAdmin) {
        leafletMap.flyTo([data.lat, data.lng], leafletMap.getZoom(), {
          animate: true,
          duration: 0.7,
        });
      }
  ...
  socket.on("map:admin-peek", (data) => {
    ...
    if (me && !me.is_superadmin) {
      showToast(
        `👁 ${data.admin_name} schaut gerade auf die gesperrte Karte`,
        "info",
      );
    }
  });
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "Enter") {
    if (drawingMode === "region" && drawingCoords.length >= 3) {
      e.preventDefault();
      finishRegionDraw();
    } else if (_fogPolyCoords?.length >= 3) {
      e.preventDefault();
      _finishFogPolygon();
    } else if (regionEditorSaveFn) {
      e.preventDefault();
      regionEditorSaveFn();
    }
  }
...
function showModeIndicator(text) {
  hideModeIndicator();
  modeEl = Object.assign(document.createElement("div"), {
...
function stopAnyDraw() {
  ...
  drawingMode = null;
  drawingCoords = [];
  tempLines.forEach((l) => leafletMap?.removeLayer(l));
  tempLines = [];
...
function refreshEditorContent() {
  if (
    !isAdmin ||
    document.getElementById("editorPanel").classList.contains("hidden")
  )
    return;
  const c = document.getElementById("editorContent");
  const builders = {
    groups: buildGroupsEditor,
    pois: buildPoisEditor,
    routes: buildRoutesEditor,
    regions: buildRegionsEditor,
    fog: buildFogEditor,
  };
...
function openEditGroup(id) {
  ...
  showFeatureModal(
    g ? "Gruppe bearbeiten" : "Neue Gruppe",
    `
    <form id="fmForm">
      ${
        shareUrl
          ? `<div class="form-group" style="background:var(--bg3);border-radius:var(--radius);padding:.6rem .75rem;margin-bottom:.8rem">
  ...
  document.getElementById("fmForm").onsubmit = async (e) => {
    ...
    const body = {
      ...
      fog_of_war_enabled: document.getElementById("fmFog").value === "true",
      external_links: gatherLinks(),
    };
    try {
...
async function deleteGroup(id) {
  ...
  try {
    await API.delete(`/api/maps/${mapData.map.id}/groups/${id}`);
  } catch (e) {
    showToast(e.message, "error");
  }
}
...
function buildPoisEditor() {
  ...
  const items = sortedPois
    .map((p) => {
      const visCls =
        { public: "vis-public", group: "vis-group", hidden: "vis-hidden" }[
          p.visibility
        ] || "vis-group";
      const visLbl =
        { public: "🌍", group: "👥", hidden: "🔒" }[p.visibility] || "👥";
  ...
  const sortOpts = [
    ...
    .map(
      ([v, l]) =>
        `<option value="${v}" ${_poiSortMode === v ? "selected" : ""}>${l}</option>`,
    )
    .join("");
...
function _sortPois(pois) {
  if (_poiSortMode === "alpha")
    return pois.sort((a, b) => a.name.localeCompare(b.name));
  if (_poiSortMode === "group")
    return pois.sort((a, b) => (a.group_id || 0) - (b.group_id || 0));
  if (_poiSortMode === "icon")
    return pois.sort((a, b) => (a.icon || "").localeCompare(b.icon || ""));
...
function startPlacePoi() {
  if (!mapData.groups.length) {
    showToast("Erst eine Gruppe erstellen", "error");
    return;
  }
  enablePickMode();
  ...
  leafletMap.once("click", (e) => {
    disablePickMode();
    hideModeIndicator();
    showPoiModal(null, e.latlng.lat, e.latlng.lng);
  });
}
...
function showPoiModal(p, lat, lng) {
  ...
  document.getElementById("pickPosBtn").onclick = () => {
    ...
    leafletMap.once("click", (ev) => {
      disablePickMode();
      hideModeIndicator();
      showPoiModal(p, ev.latlng.lat, ev.latlng.lng);
    });
  };
  document.getElementById("fmForm").onsubmit = async (e) => {
    e.preventDefault();
    ...
    const body = {
      ...
      lat: +document.getElementById("fmLat").value,
      lng: +document.getElementById("fmLng").value,
      icon:
        document.querySelector(".icon-option.selected")?.dataset.icon ||
        "unknown",
      color: document.getElementById("fmColor").value,
      bg_transparent: document
        .getElementById("fmTransparentSwatch")
...
function toggleTransparentSwatch() {
  ...
  if (col) {
    col.style.opacity = sel ? ".35" : "1";
    col.style.pointerEvents = sel ? "none" : "";
  }
...
function iconGrid(current = "unknown") {
  ...
  const selCust =
    !isNaN(custId) && custId > 0 && _customIcons.find((ic) => ic.id === custId);
  let selHtml;
  if (selCust) {
    const isUrl =
      selCust.image_url.startsWith("http") || selCust.image_url.startsWith("/");
    selHtml = isUrl
  ...
  for (const [key, grpData] of Object.entries(POI_ICONS_GROUPED)) {
    ...
    const cells = Object.entries(iconsMap)
      .map(
        ([k, v]) =>
  ...
  if (_customIcons.length) {
    ...
    const cells = _customIcons
      .map((ic) => {
        const isUrl =
          ic.image_url.startsWith("http") || ic.image_url.startsWith("/");
        const inner = isUrl
...
function selectIcon(el) {
  ...
  const lbl = document.querySelector(
    '.icon-picker-selected span[style*="flex"]',
  );
...
async function startQuickAddWaypoint(routeId) {
  ...
  enablePickMode();
  showModeIndicator(
    "📍 Klick auf Karte: +Wegpunkt (Shift halten für mehrere) · ESC abbrechen",
  );
  ...
  const handlePointClick = async (ev) => {
    ...
    const nextIdx = existing.length
      ? Math.max(...existing.map((w) => +w.order_index)) + 1
      : 0;
...
function insertWaypointAfter(routeId, afterWpId, afterIdx) {
  ...
  enablePickMode();
  showModeIndicator(
    "📍 Klick auf Karte für Zwischenpunkt vorher · ESC abbrechen",
  );
  leafletMap.once("click", async (ev) => {
    disablePickMode();
    hideModeIndicator();
    try {
      ...
      await API.post(
        `/api/maps/${mapData.map.id}/routes/${routeId}/waypoints`,
        {
          lat: ev.latlng.lat,
          lng: ev.latlng.lng,
          title: "",
          info: "",
          order_index: afterIdx - 0.5,
        },
      );
...
function buildRegionsEditor() {
  const items = mapData.regions
    .map((r) => {
      ...
      const visLbl =
        { public: "🌍", group: "👥", hidden: "🔒" }[r.visibility] || "👥";
...
function startDrawRegion() {
  ...
  showModeIndicator(
    "🏔 Punkte klicken · Enter/Doppelklick fertig · ESC abbrechen",
  );
...
function editRegionVertices(id) {
  ...
  regionEditor = new RegionVertexEditor(leafletMap, coords, (c) => {
    coords = c;
  });

  showModeIndicator(
    "🔷 Punkte ziehen · Klick auf Mitte: einfügen · Rechtsklick: löschen · Enter oder ✓ zum Speichern",
  );
  ...
  doneBtn.style.cssText =
    "position:absolute;bottom:70px;right:12px;z-index:450;box-shadow:var(--shadow)";
  ...
  const saveFn = async () => {
    ...
    try {
      await API.put(`/api/maps/${mapData.map.id}/regions/${r.id}`, {
        ...
        stroke_opacity: r.stroke_opacity,
...
function showRegionModal(r, coords) {
  showFeatureModal(
    r ? "Region bearbeiten" : "Neue Region",
    `
    <form id="fmForm">
      <div class="form-group"><label>Name *</label><input id="fmName" value="${escAttr(r?.name || "")}" required></div>
      <div class="form-group"><label>Beschreibung</label><div id="fmDescQuill" class="quill-container"></div></div>
  ...
  document.getElementById("fmForm").onsubmit = async (e) => {
    ...
    const body = {
      ...
      visibility,
      name: document.getElementById("fmName").value,
      description: getQuillHtml(_rQuill),
      coordinates: coords,
      color: document.getElementById("fmColor").value,
      fill_opacity: +document.getElementById("fmFill").value,
      stroke_opacity: 0.85,
    };
    try {
...
function buildFogEditor() {
  ...
  const items = fogGs
    .map((g) => {
      const areas = (mapData.fog_areas || []).filter(
        (a) => a.group_id === g.id,
      );
...
function startFogPolygon(groupId) {
  ...
  enablePickMode();
  showModeIndicator(
    "🌫 Nebelbereich zeichnen: Klicken · Enter/Doppelklick fertig · ESC abbrechen",
  );
...
function _onFogPolyClick(e) {
  ...
  if (_fogPolyCoords.length >= 3)
    fogCanvas.addPreviewArea(_fogPolyGroupId, _fogPolyCoords);
}

function _onFogPolyDblClick(e) {
  L.DomEvent.stopPropagation(e);
  _finishFogPolygon();
}
...
async function _finishFogPolygon() {
  ...
  _setActiveFogBtn(null);
  if (coords.length < 3) {
    showToast("Mindestens 3 Punkte erforderlich", "error");
    return;
  }
...
async function deleteFogArea(areaId, groupId) {
  ...
  try {
    await API.delete(`/api/maps/${mapData.map.id}/fog/areas/${areaId}`);
  } catch (e) {
    showToast(e.message, "error");
  }
}
...
function unhighlightFogArea() {
  if (_fogHighlightLayer) {
    leafletMap.removeLayer(_fogHighlightLayer);
    _fogHighlightLayer = null;
  }
}
...
class RegionVertexEditor {
  ...
  _render() {
    ...
    this.coords.forEach((c, idx) => {
      ...
      m.bindTooltip(`Punkt ${idx + 1} · Rechtsklick löschen`, {
        permanent: false,
        opacity: 0.8,
      });
    });
    ...
    this.coords.forEach((c, idx) => {
      ...
      this._layers.push(m);
      m.on("click", () => {
        this.coords.splice(idx + 1, 0, [...mid]);
        this._render();
        this.onChange(this.coords);
      });
      m.bindTooltip("Klick: Punkt einfügen", {
        permanent: false,
        opacity: 0.8,
      });
    });
...
function addLinkRow() {
  document
    .getElementById("linksList")
    .insertAdjacentHTML("beforeend", linkRow());
}
function gatherLinks() {
  return [...document.querySelectorAll("#linksList .ll")]
    .map((el, i) => {
...
function _toggleQuillSource(q, editorEl, btn) {
  ...
  if (!q._sourceMode) {
    ...
    if (!q._sourceTa) {
      ...
      q._sourceTa.style.cssText =
        "width:100%;min-height:100px;font-family:monospace;font-size:12px;background:var(--bg3);color:var(--text);border:1px solid var(--border);border-radius:var(--radius);padding:.5rem;resize:vertical";
...
function initScaleBar(mapCfg) {
  ...
  function update() {
    ...
    if (totalMiles >= 100)
      display =
        Math.round(totalMiles) + " Meilen (" + Math.round(totalKm) + " km)";
    else if (totalMiles >= 10)
      display =
        Math.round(totalMiles) + " Meilen (" + Math.round(totalKm) + " km)";
    else if (totalMiles >= 1)
      display =
        totalMiles.toFixed(1) + " Meilen (" + totalKm.toFixed(1) + " km)";
...
function startMeasure(mapCfg) {
  ...
  showModeIndicator(
    "📏 Linksklick: Punkt · Rechtsklick: Beenden · ESC: Abbruch",
  );
}
...
function _redrawMeasure(mapCfg) {
  ...
  _showMeasurePanel(
    `<strong>${distText}</strong><br><span style="color:var(--text-dim);font-size:12px">${travelText}</span><br><span style="color:var(--text-dim);font-size:11px">${pts.length} Punkte · Doppelklick zum Beenden</span>`,
    mapCfg,
  );
}
...
function _poiIconHtml(iconKey) {
  ...
  if (ci) {
    const isUrl =
      ci.image_url.startsWith("http") || ci.image_url.startsWith("/");
    return isUrl
...
function _handleFeaturePing(data) {
  ...
  if (type === "poi") {
    ...
    layer = L.polygon(r.poly.getLatLngs(), {
      color: "#f97316",
      fillColor: "#f97316",
      fillOpacity: 0.35,
      weight: 4,
  ...
  if (!leafletMap.getBounds().contains(center)) {
    leafletMap.flyTo(center, leafletMap.getZoom(), {
      animate: true,
      duration: 0.8,
    });
  }
  ...
  _pingInterval = setInterval(() => {
    ...
    try {
      if (type === "poi")
        layer.setStyle({ fillOpacity: on ? 0.5 : 0.1, opacity: on ? 1 : 0.3 });
      else if (type === "region")
        layer.setStyle({
...
function _renderPoiLabels() {
  ...
  mapData.pois.forEach((p) => {
    ...
    L.marker([+p.lat, +p.lng], { icon: lbl, interactive: false }).addTo(
      _poiLabelGroup,
    );
  });
...
const _REG_LABEL_URL =
  "https://www.9ps.eu/dnd/items/Worldmapper/label_region.webp";
...
// Oben im globalen Bereich der Datei (ca. bei Zeile 89ff)
let _REG_LABEL_W = 210;
let _REG_LABEL_H = Math.round((210 * 222) / 616);
...
function _renderRegionLabels() {
  ...
  const regFont = ds.region_font
    ? ds.region_font.replace(/\.[^/.]+$/, "")
    : "MorrisRoman";

  // NUTZE HIER DIE GLOBALEN VARIABLEN (ohne "_" davor neu zu deklarieren)
  const w = _REG_LABEL_W;
  const h = _REG_LABEL_H;
  ...
  mapData.regions.forEach((region) => {
    ...
    try {
      ...
      L.marker(center, { icon: lbl, interactive: false }).addTo(_regLabelGroup);
    } catch (err) {
      console.warn(
        "Fehler beim Erstellen des Labels für Region:",
        region.name,
        err,
      );
    }
...
function focusPoi(id) {
  const p = mapData.pois.find((x) => x.id === id);
  if (!p) return;
  leafletMap.flyTo([+p.lat, +p.lng], leafletMap.getZoom(), {
    animate: true,
    duration: 0.5,
  });
  highlightPoi(id);
...
function focusRoute(id) {
  ...
  if (!coords.length) return;
  if (coords.length === 1)
    leafletMap.flyTo(coords[0], leafletMap.getZoom(), {
      animate: true,
      duration: 0.5,
    });
  else
    leafletMap.flyToBounds(L.polyline(coords).getBounds(), {
      animate: true,
      duration: 0.5,
    });
  highlightRoute(id);
...
function bringRegionToFront(id) {
  ...
  if (e) {
    e.poly.bringToFront();
    showToast("Region nach vorne gebracht");
  }
}
function sendRegionToBack(id) {
  ...
  if (e) {
    e.poly.bringToBack();
    showToast("Region nach hinten verschoben");
  }
}
...
function _showLockButton(isLocked) {
  ...
  if (!btn) {
    ...
    document
      ...
      .insertBefore(btn, document.getElementById("toggleEditorBtn"));
  }
  if (isLocked) {
...
function _showAdminLockBar(state) {
  ...
  const lockedAt = state.locked_at
    ? new Date(state.locked_at).toLocaleTimeString("de")
    : "";
  bar.innerHTML = `
...
async function lockMap() {
  ...
  if (
    !confirm(
      "Karte für den Admin sperren?\nDer Admin sieht die Karte dann nur als Betrachter.\nEr wird benachrichtigt, wenn er die Karte im Editor öffnet.",
    )
  )
    return;
  try {
...
async function adminForceUnlock() {
  if (!confirm("Sperre aufheben? Der Editor wird dabei nicht benachrichtigt."))
    return;
  try {
    await API.delete(`/api/maps/${mapData.map.id}/lock`);
