    ...
    grid.innerHTML = `<div class="empty-state">
      ...
      <p>${
        ME.is_superadmin
          ...
          : "Dir wurden noch keine Karten zugewiesen."
      }</p>
    </div>`;
    return;
  }
...
async function deleteMap(id, name) {
  if (
    !confirm(
      `Karte „${name}" wirklich löschen?\nAlle Gruppen, POIs, Routen und Regionen werden entfernt.`,
    )
  )
    return;
  try {
...
async function openSettings(id) {
  ...
  document.getElementById("defPoiLabelSize").value =
    parseInt(ds.poi_label_size) || 22;
  document.getElementById("defPoiColor").value =
    ds.poi_label_color || "#e2e8f0";
  document.getElementById("defFontRegion").value = ds.region_font || "";
  document.getElementById("defRegionLabelSize").value =
    parseInt(ds.region_label_size) || 32;
  document.getElementById("defRegionColor").value =
    ds.region_label_color || "#e2e8f0";
  document.getElementById("defPoiBorder").value =
    ds.poi_border_color || "#bcbcbc";
  document.getElementById("defPingDur").value = ds.ping_duration || 5;
  document.getElementById("defRegionWidth").value =
    parseInt(ds.region_label_width) || 210;
  ...
  // Map share link
  document.getElementById("shareUrlMap").value =
    `${location.origin}/map.html?t=${m.share_token}`;

  // Scale & travel fields
  document.getElementById("mapScaleLabel").value = m.map_scale_label || "";
  document.getElementById("mapMilesWidth").value =
    m.map_miles_width != null ? m.map_miles_width : "";
  document.getElementById("mapMilesPerDay").value =
    m.travel_miles_per_day != null ? m.travel_miles_per_day : 24;
  document.getElementById("mapHoursPerDay").value =
    m.travel_hours_per_day != null ? m.travel_hours_per_day : 8;
...
async function saveGeneralSettings() {
  ...
  const ds = {
    ...
    region_label_size:
      document.getElementById("defRegionLabelSize").value + "px",
...
async function loadGroupShareLinks(mapId) {
  try {
    ...
    if (!gs.length) {
      el.innerHTML = '<p class="hint">Noch keine Gruppen angelegt.</p>';
      return;
    }
    el.innerHTML = gs
      .map(
...
async function handleRestore(input) {
  ...
  if (
    !confirm(
      "Möchtest du das Backup wirklich einspielen? Bestehende Daten werden überschrieben!",
    )
  ) {
    input.value = "";
    return;
  ...
  try {
    ...
    showToast(
      "Backup erfolgreich eingespielt. Seite wird neu geladen...",
      "success",
    );
...
async function saveMapScale() {
  ...
  try {
    await API.put(`/api/maps/${settingsMapId}`, {
      ...
      map_scale_label:
        document.getElementById("mapScaleLabel").value.trim() || null,
      map_miles_width:
        parseFloat(document.getElementById("mapMilesWidth").value) || null,
      travel_miles_per_day:
        parseFloat(document.getElementById("mapMilesPerDay").value) || 24,
      travel_hours_per_day:
        parseFloat(document.getElementById("mapHoursPerDay").value) || 8,
    });
...
async function loadFontList() {
  try {
    ...
    selects.forEach((id) => {
      ...
      if (!sel) return;
      sel.innerHTML =
        '<option value="">Standard-Systemschrift</option>' +
        fonts
          .map(
            (f) =>
              `<option value="${f}">${f.replace(/\.(ttf|otf)$/i, "")}</option>`,
          )
          .join("");
    });
  } catch (e) {
...
function updateFontPreview(type) {
  ...
  const fontFile = document.getElementById(
    isPoi ? "defFontMain" : "defFontRegion",
  ).value;
  const size = document.getElementById(
    isPoi ? "defPoiLabelSize" : "defRegionLabelSize",
  ).value;
  const color = document.getElementById(
    isPoi ? "defPoiColor" : "defRegionColor",
  ).value;
  const previewContainer = document.getElementById(
    isPoi ? "fontPreviewPoi" : "fontPreviewRegion",
  );

  if (!previewContainer) return;

  const textTarget = isPoi
    ? previewContainer
    : previewContainer.querySelector(".region-preview-text");
  ...
  // --- NEU: Größen-Anpassung nur für Regionen ---
  if (!isPoi) {
    const regWidthInput = document.getElementById("defRegionWidth");
    if (regWidthInput) {
      const regWidth = parseInt(regWidthInput.value);

      // 1. Breite setzen
      previewContainer.style.width = regWidth + "px";

      // 2. Höhe EXAKT nach dem Originalverhältnis (616x222) berechnen
      // Das verhindert das Stauchen, da Breite und Höhe immer synchron bleiben
      const calculatedHeight = Math.round((regWidth * 222) / 616);
      previewContainer.style.height = calculatedHeight + "px";

      // 3. Den Text-Container mitskalieren (optionaler Feinschliff)
      // Falls der Text bei kleinen Breiten zu groß wirkt:
      if (textTarget) {
        textTarget.style.width = "63%"; // Behält das Verhältnis im Bild bei
        textTarget.style.height = "50%";
      }
    }
  }
}
  ...
  try {
    ...
    document.getElementById("editorsList").innerHTML =
      admins
        ...
        .join("") ||
      '<p style="color:var(--text-dim);font-size:13px">Keine weiteren Admins</p>';
    ...
    sel.innerHTML =
      ...
      users
        .filter((u) => !adminIds.has(u.id) && u.id !== mapOwnerId)
        .map((u) => `<option value="${u.id}">${escHtml(u.username)}</option>`)
        .join("");
  } catch {}
}
  ...
  try {
    ...
    document.getElementById("usersList").innerHTML =
      users
        ...
        .join("") || '<p style="color:var(--text-dim)">Keine Benutzer</p>';
  } catch {}
}
...
function openAdminPwModal(userId, username) {
  ...
  document.getElementById("adminPwTitle").textContent =
    `Passwort für „${username}" setzen`;
...
function iconPreviewHtml(url, name) {
  if (isIconUrl(url))
    return `<img src="${escHtml(url)}" alt="${escHtml(name)}" onerror="this.style.opacity='.3'" style="width:36px;height:36px;object-fit:contain;border-radius:4px">`;
...
function iconDisplayName(icon) {
  ...
  return escHtml(
    icon.name.startsWith(prefix) ? icon.name.slice(prefix.length) : icon.name,
  );
}
...
function renderIconsList() {
  ...
  if (!allIcons.length) {
    el.innerHTML =
      '<p style="color:var(--text-dim);font-size:13px">Noch keine Icons.</p>';
    return;
  }
  ...
  const others = allIcons.filter(
    (i) => i.owner_id !== null && i.owner_id !== ME.id,
  );
  ...
  return `<div class="icon-manage-item" id="icon-row-${icon.id}">
    ...
    <div style="display:flex;gap:.3rem;flex-shrink:0">
      ${
        readOnly
          ? ""
          : `<button class="btn btn-ghost btn-sm" onclick="editIcon(${icon.id})">✏️</button>
...
async function createIcon() {
  ...
  if (!name || !url) {
    showToast("Name und Emoji/URL erforderlich", "error");
    return;
  }
  try {
...
function editIcon(id) {
  ...
  const dname = ME.is_superadmin
    ? icon.name
    : icon.name.replace(new RegExp("^" + ME.username + "_"), "");
  row.innerHTML = `
    <div style="width:40px;height:40px;display:flex;align-items:center;justify-content:center;background:var(--border);border-radius:4px;flex-shrink:0">
      ${iconPreviewHtml(icon.image_url, icon.name)}
    </div>
  ...
  if (el) {
    el.innerHTML = _changelogHtml
      ? typeof DOMPurify !== "undefined"
        ? DOMPurify.sanitize(_changelogHtml)
        : _changelogHtml
...
async function saveChangelog() {
  ...
  try {
    ...
    const el = document.getElementById("changelogDisplay");
    if (el)
      el.innerHTML = clean || '<p style="color:var(--text-dim)">Leer.</p>';
    cancelChangelogEdit();
...
async function loadAuditLog(offset = 0) {
  ...
  try {
    const d = await API.get(
      url
        .replace("/api/audit-log", "audit-log")
        .replace("audit-log", "/api/audit-log"),
    );
...
function renderAuditRows(rows) {
  ...
  if (!rows.length) {
    tbody.innerHTML =
      '<tr><td colspan="5" style="text-align:center;color:var(--text-dim);padding:1.5rem">Keine Einträge</td></tr>';
    return;
  }
  tbody.innerHTML = rows
    .map((r) => {
      const dt = new Date(r.created_at).toLocaleString("de");
      const actionCls = r.action.startsWith("DELETE")
        ? "color:#fca5a5"
        : r.action.startsWith("LOGIN")
          ? "color:#86efac"
          : "";
      return `<tr>
...
loadMaps().then(() => {
  if (ME.is_superadmin || true) loadMapStats();
});
